document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculateCombinationTherapy = (weight, ferritin) => {
        ...
        if (selectedDrugs.length === 3) {
            ...
            const dfoTotal = Math.round(weight * Math.min(doseMap.dfo, 60));
            const dfxResult = findTabletCombination(weight * Math.min(doseMap.dfx, 28), [360, 180, 90], 90);
            doseDetails.innerHTML = `<div class="combo-result"><span><strong>دفروکسامین:</strong> ${dfoTotal} میلی‌گرم (${getVialText(dfoTotal).replace('معادل ','')})</span><span class="dose-per-kg-text">(بر اساس ${doseMap.dfo} mg/kg)</span><span class="combo-days">${doseMap.dfoDays} روز در هفته</span></div>`
                                  + `<div class="combo-result"><span><strong>دفراسیروکس:</strong> ${dfxResult.totalDose} میلی‌گرم (${dfxResult.combination})</span><span class="dose-per-kg-text">(بر اساس ${doseMap.dfx} mg/kg)</span><span class="combo-days">در روزهای دیگر</span></div>`;
